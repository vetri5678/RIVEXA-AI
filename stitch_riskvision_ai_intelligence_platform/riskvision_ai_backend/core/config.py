...
class Settings(BaseSettings):
    ...
    # Spring Boot backend JWT secret — used for dual-decode of tokens created by the Java backend.
    # Must match the jwt.secret property in application.properties / SECRET_KEY env var.
    # REQUIRED: Set SPRINGBOOT_JWT_SECRET env var to the same value as SECRET_KEY in springboot .env
    springboot_jwt_secret: str = Field(
        default="",
        alias="SPRINGBOOT_JWT_SECRET",
    )

    # CORS
    cors_origins: List[str] = Field(
        default=["http://localhost:5176", "http://127.0.0.1:5176", "http://localhost:3000"],
        alias="CORS_ORIGINS",
    )

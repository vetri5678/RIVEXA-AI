...
def download_pdf(
    ...
    content = ReportService.generate_pdf(
        db, current_user.id, prediction_id, project_id, get_client_ip(request),
    )
    filename = f"risk_report_{prediction_id or project_id or 'export'}.pdf"
...
def download_excel(
    ...
    filename = f"risk_report_{prediction_id or project_id or 'export'}.xlsx"
    return StreamingResponse(
        io.BytesIO(content),

...
function initRouter() {
  async function handleRoute() {
    ...
    let hashWithoutHash = rawHash.replace(/^#/, '');

    // Extract route path and query parameters
    let route = hashWithoutHash.split('?')[0];
    let queryStr = hashWithoutHash.includes('?')
    ...
    if (route === '/oauth2/callback' || route === '/auth/oauth-success') {
      ...
      if (errorMsg) {
        console.error('[OAuth Callback] Error parameter received:', errorMsg);
        showToast('OAuth Error: ' + decodeURIComponent(errorMsg), 'error');
        window.location.hash = '#/login?error=' + encodeURIComponent(errorMsg);
        return;
      }

      if (token) {
        console.log('[OAuth Callback] Received valid JWT token, saving to localStorage...');
    ...
    let pageId = routes[route];
    if (!pageId) {
      pageId = isLoggedIn() ? 'page-dashboard' : 'page-home';
    }
    ...
    const protectedPages = ['page-dashboard', 'page-projects', 'page-predictions', 'page-training', 'page-notifications', 'page-profile', 'page-settings', 'page-admin'];
    if (!isLoggedIn() && protectedPages.includes(pageId)) {
      console.log('[Router] Access to protected page denied. Redirecting to Login.');
      window.location.hash = '#/login';
      return;
    }
...
function bindFormHandlers() {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
      ...
      console.log('[Auth] Initiating static login request...', { email: emailNormalized });

      try {
        setButtonLoading(btn, true, 'Authenticating...');
        const { rvLogin, rvGetMe } = await import('./api.js');
        await rvLogin(emailNormalized, password);
        const user = await rvGetMe();
        localStorage.setItem('rv_user', JSON.stringify(user));
        refreshAuthState();
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', async (e) => {
      ...
      console.log('[Auth] Initiating static registration request...', { email: emailNormalized, username, full_name: fullName });

      try {
        setButtonLoading(btn, true, 'Creating Account...');
        const { rvRegister } = await import('./api.js');
        await rvRegister(emailNormalized, username, password, fullName);
  ...
  if (forgotForm) {
    forgotForm.addEventListener('submit', async (e) => {
      ...
      console.log('[Auth] Initiating static password reset request...', { email: emailNormalized });

      try {
        setButtonLoading(btn, true, 'Sending...');
        const { rvRequestPasswordReset } = await import('./api.js');
        await rvRequestPasswordReset(emailNormalized);
...
window.handleLoginSubmit = async function(event) {
  ...
  try {
    ...
    if (submitBtn) {
      ...
      submitBtn.innerHTML = '<span>Sign In</span><span class="material-symbols-outlined text-xl">arrow_forward</span>';
    }
  }
};

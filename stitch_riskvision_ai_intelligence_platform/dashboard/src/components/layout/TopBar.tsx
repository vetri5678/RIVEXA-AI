...
import { Search, Bell, Play, RefreshCw, User as UserIcon, Command } from 'lucide-react';
...
export const TopBar: React.FC<TopBarProps> = ({
  ...
  const user = userRaw ? JSON.parse(userRaw) : { full_name: 'Administrator', role: 'SUPER_ADMIN', email: '' };

  return (
    <header
      className={`h-16 bg-[#050816]/85 backdrop-blur-2xl border-b border-white/[0.08] fixed top-0 right-0 z-20 flex items-center justify-between px-6 transition-all duration-300 shadow-lg ${
    ...
    >
      ...
      <div className="relative w-80 md:w-96">
        ...
        <input
          ...
          placeholder="Global Search (Repos, Predictions, Logs)..."
          className="w-full pl-10 pr-12 py-2 bg-white/[0.04] border border-white/[0.08] rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500/60 focus:ring-2 focus:ring-blue-500/20 focus:bg-white/[0.06] transition-all duration-200"
        />
        <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
          <kbd className="hidden sm:inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[10px] font-mono font-medium text-slate-400 bg-white/[0.08] border border-white/[0.1] rounded">
            <Command size={10} /> K
          </kbd>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-xs text-emerald-400 font-mono">
          ...
          <span className="font-semibold text-[11px]">XGBoost v2.4 · Operational</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="btn-primary text-xs py-1.5 px-3 flex items-center gap-1.5 rounded-xl cursor-pointer"
          >
          ...
          <button
            ...
            className="btn-secondary text-xs py-1.5 px-3 flex items-center gap-1.5 rounded-xl cursor-pointer"
          >

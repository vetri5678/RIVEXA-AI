            ...
            if (res && 'writeHead' in res) {
              ...
              if (!httpRes.headersSent) {
                ...
                httpRes.end(JSON.stringify({
                  ...
                  message: `Spring Boot server unreachable at http://127.0.0.1:8080. (${err.message})`,
                  path: req.url,
                }))
              }
            }
          ...
          proxy.on('error', (err, req, res) => {
            ...
            if (res && 'writeHead' in res) {
              ...
              if (!httpRes.headersSent) {
                ...
                httpRes.end(JSON.stringify({
                  error: 'Service Unavailable (ECONNREFUSED)',
                  message: `Backend server unreachable at http://127.0.0.1:8080. Please start the backend service. (${err.message})`,
                  path: req.url,
                }))
              }
            }

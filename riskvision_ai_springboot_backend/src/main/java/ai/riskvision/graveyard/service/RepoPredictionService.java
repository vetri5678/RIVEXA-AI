...
public class RepoPredictionService {
    ...
    public RepositoryPredictionEntity runPrediction(UUID repositoryId, String actor) {
        ...
        try {
            log.info("Attempting ML prediction via Python backend at: {}", mlServiceUrl);
            ...
            // Map engineering parameters to ML training schema inputs
            int contributorsCount = entity.getContributors() != null ? entity.getContributors() : 0;
            request.put("actual_cost", contributorsCount * 12000.0);
            request.put("timeline_months", 12.0);
            request.put("actual_duration", 3.0);
            request.put("team_size", contributorsCount > 0 ? (double) contributorsCount : 1.0);
            request.put("status", "active");
            request.put("requirements_changed", 0.0);
            request.put("total_requirements", 10.0);
            request.put("features_delivered", 8.0);
            request.put("identified_risks", entity.getOpenIssues() != null ? (double) entity.getOpenIssues() : 0.0);
            request.put("total_tasks", 50.0);
            ...
            if (response != null) {
                ...
                log.info("ML Prediction succeeded. failureProbability={}, riskLevel={}", failureProb, riskLevel);
            } else {
                throw new RuntimeException("Empty response received from ML Service");
            }
        } catch (Exception e) {
            log.warn("Failed to contact Python ML Service ({}). Falling back to heuristic engine. Error: {}", mlServiceUrl, e.getMessage());
